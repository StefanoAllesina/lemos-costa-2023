# Plots for model 2 (would also apply to model 3)
rm(list = ls())
library(ggstance)
library(tidyverse)
library(ggtree)
...
fn <- "../organized_results/bioII_2008-7_2.RData"
fn_name <- "Biodiversity II (2008) model 2"
...
plot_tree_m2 <- function(resultsfile, scalefac, label){
  load(resultsfile)
  ...
  df_bar_data <- tibble(id = sp_names,
                        bar_value = results$pars_orig[(2*n):(length(results$pars_orig)-1)])
  ...
  pl <- ggtree(tree, aes(colour = scalefac * log(branch.length)), 
    ...
    geom_facet(panel = expression(gamma), data = df_bar_data, geom = geom_barh,stat='identity',
               aes(x = bar_value), width = .6, fill="#F7A90C", color = NA) +
    theme(legend.position = "none") + ggtitle(label)
  return(pl)
}


pl_tree <- plot_tree_m2(fn, 3, fn_name)
pl_tree

## Creating all the plots
files <- list.files(path = "../organized_results/", pattern = "\\_2.RData$")
...
for(i in 1:length(fn)){
  pl_tree <- plot_tree_m2(fn[i], 3, paste0(fn_name[i], " model 2"))
  ggsave(filename = paste0("figures_models_2-4/", fn_name[i], "_m2.pdf"), plot = pl_tree)
  print(i)
}
